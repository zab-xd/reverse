...
def bans():
    ...
    | Script by : Master Newbie 😎  |
    | Telegram  : @Master_Newbie    |
    |_______________________________|
    'Updated Free Version 7'
    """
    for char in banner:
        sys.stdout.write(str(char))
        sys.stdout.flush()
...
def md(cu):
    try:
        ...
        connector = 'connect'
        if str(e).__contains__('seconds is required'):
            try:
                i = re.findall('[0-9]+',str(e))[0]
                print(" \033[0;1;31m Waits error [ "+str(i)+" seconds]")
            except:
                pass
        try:
            account.disconnect()
...
def auto_leave():
    ...
    try:
        ...
        for dialog in account.get_dialogs():
            ...
            try:
                channel_entity = account.get_entity(PeerChannel(int(dialog.entity.id)))
                print(" "+ channel_entity.title + "\r",end="")
                account(LeaveChannelRequest(channel_entity))
                i+=1
                print(" [" +str(i)+ "] " + channel_entity.title + " Leave Success")
            except:
                pass
        print(" Done Leave                      ")
    except Exception as e:
...
def out_reward():
    i=0
    while True:
        reward = account(GetHistoryRequest(peer=ltc,limit=2,offset_date=None,offset_id=0,max_id=0,min_id=0,add_offset=0,hash=0)).messages
        r = reward[0].message
        r1 = reward[1].message
            ...
            for x in w:
                if x.__contains__("You must stay"):
                    try:
                        ...
                        print("\033[0;1;35m[\033[0;1;34m+\033[0;1;35m]\033[0;1;37m Stay at least "+str(sxfefa)+" hour \033[0;1;37m                    ")
                    except:
                        print(x)
            break
        if r.__contains__('looks invalid'):
            ...
            for x in w:
                if x.__contains__("You must stay"):
                    try:
                        sxfefa = re.findall("[0-9]+",x)[0]
                        print(" \033[0;1;35m[\033[0;1;34m+\033[0;1;35m]\033[0;1;37mStay at least "+str(sxfefa)+" hour                      ")
                    except:
                        print(x)
            break
        ...
        sleep(1)
        if i >= 5:
            print(" \033[0;1;31mBot Server to Slow\033[0;1;37m                            ")
            break

def auto_join():
    print("\033[0;1;34;40m Auto Join Chats\033[0m                                   ")
    print("===============================")
    waitsError = 0
    lim=0
    try:
        ...
        messagejoin = aaxx['jm']
        entity_link = aaxx['ml']
        messagefirst = aaxx['m1']
        if str(server) == 'on':
            try:
                if int(trigstart) == 1:
                    iop = False
                    ssssss = account(GetHistoryRequest(peer=main_link_entity,limit=3,offset_date=None,offset_id=0,max_id=0,min_id=0,add_offset=0,hash=0)).messages
                    for gsd in ssssss:
                        if str(gsd.message).__contains__('/start'):
                            iop = True
                            pass
                    if iop == False:
                        account(StartBotRequest(bot=entity_link,peer=entity_link,start_param=refcode))
                else:
                    account(StartBotRequest(bot=entity_link,peer=entity_link,start_param=refcode))
            except:
                account(StartBotRequest(bot=entity_link,peer=entity_link,start_param=refcode))
            if len(joinbotlistlink) > 0:
                for ja in joinbotlistlink:
                    try:
                        account(JoinChannelRequest(ja))
                    except:
                        pass
            try:
                if len(invitejoin[0]) > 2:
                    for cxz in invitejoin:
                        account(ImportChatInviteRequest(cxz))
            except:
                pass
            try:
                main_link_entity = account.get_entity(usernamelink)
                history = account(GetHistoryRequest(peer=main_link_entity,limit=2,offset_date=None,offset_id=0,max_id=0,min_id=0,add_offset=0,hash=0))
                ids = history.messages[0].id
                try:
                    if str(messagefirst) == '1':
                        if len(messagejoin) >= 2:
                            account.send_message(entity=main_link_entity,message=messagejoin)
                except:
                    pass
                account(GetBotCallbackAnswerRequest(usernamelink, ids,data=history.messages[0].reply_markup.rows[0].buttons[0].data))
            except:
                try:
                    if len(messagejoin) >= 2:
                        account.send_message(entity=main_link_entity,message=messagejoin)
    ...
    while True:
        try:
            ...
            account(GetBotCallbackAnswerRequest(ltc_username, join_id, data=history.messages[0].reply_markup.rows[0].buttons[1].data))
            sleep(1)
            out_reward()
            r = account(GetHistoryRequest(peer=ltc,limit=2,offset_date=None,offset_id=0,max_id=0,min_id=0,add_offset=0,hash=0)).messages[0].message
            ...
            if str(e).__contains__("seconds is "):
                try:
                    ...
                    print(' \033[0;1;31mTry again after '+str(xxeess[0])+' seconds\033[0;1;37m')
                except:
                    pass
                break
            else:
...
def mess_bots():
    ...
    try:
        ...
        xrefcode = aaxx['rlo']
        if str(serverl) == '1':
            account(StartBotRequest(bot=xlinko,peer=xlinko,start_param=xrefcode))
    except:
        pass
    while True:
        sleep(1)
